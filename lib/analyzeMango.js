...
const RIPENESS_DISHES = {
  Unripe: ["Mango pickle", "Raw mango chutney", "Aam panna"],
  "Semi-ripe": ["Mango chaat", "Mango salsa", "Semi-ripe mango chutney"],
  Perfect: ["Mango shake", "Aamras", "Mango smoothie"],
  Overripe: ["Mango cake", "Mango jam", "Mango dessert"],
  Spoiled: ["Do not consume", "Discard safely", "Use a fresh mango"]
};
const RIPENESS_RECIPES = {
  Unripe: [
    {
      name: "Raw Mango Chutney",
      steps: [
        "Peel and chop the firm raw mango.",
        "Blend with mint, coriander, green chili, and salt.",
        "Add roasted cumin and a little jaggery.",
        "Serve fresh with snacks or meals."
      ]
    },
    {
      name: "Quick Mango Pickle",
      steps: [
        "Cut raw mango into small pieces.",
        "Mix with salt, turmeric, chili, and mustard seeds.",
        "Warm mustard oil and cool slightly.",
        "Toss everything together and rest before serving."
      ]
    }
  ],
  "Semi-ripe": [
    {
      name: "Mango Chaat",
      steps: [
        "Dice the semi-ripe mango.",
        "Add onion, coriander, chili, and chaat masala.",
    ...
    },
    {
      name: "Mango Salsa",
      steps: [
        "Chop mango, tomato, onion, and coriander.",
        "Add salt, chili, and lime juice.",
        "Rest for 5 minutes.",
        "Serve with chips or grilled food."
      ]
    }
  ],
  Perfect: FALLBACK_ANALYSIS.recipes,
  Overripe: [
    {
      name: "Mango Jam",
      steps: [
        "Scoop the soft mango pulp.",
        "Cook with sugar and lemon juice on low heat.",
    ...
    },
    {
      name: "Mango Cake",
      steps: [
        "Mash the over-ripe mango into pulp.",
        "Fold into a simple vanilla cake batter.",
        "Bake until the center sets.",
        "Cool and serve with cardamom cream."
      ]
    }
  ],
  Spoiled: [
    {
      name: "Safe Discard",
      steps: [
        "Do not taste mango with mold, leakage, or rotten patches.",
        "Seal it in a waste bag.",
        "Clean the storage surface.",
        "Use a fresh mango for cooking."
      ]
    },
    {
      name: "Fresh Mango Replacement",
      steps: [
        "Choose a mango without dark lesions or leakage.",
        "Wash and dry the fruit.",
        "Scan again for a fresh result.",
        "Prepare the recipe only if quality looks safe."
      ]
    }
  ]
};
...
export function normalizeMangoAnalysis(payload = {}, options = {}) {
  ...
  const fallbackDishes = RIPENESS_DISHES[ripeness] || fallback.dishes;
  const fallbackRecipes = RIPENESS_RECIPES[ripeness] || fallback.recipes;
  const dishes =
    ripeness === "Spoiled"
      ? fallbackDishes
      : normalizeList(payload.dishes, fallbackDishes).slice(0, 4);
  const recipes =
    ripeness === "Spoiled"
      ? fallbackRecipes
      : normalizeRecipes(payload.recipes, fallbackRecipes);
